    ...
    <div className="min-h-full font-[Inter]">
      ...
      <header className="hero border-b border-white/5">
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-16 min-h-[70vh] flex items-center justify-center">
          <div className="absolute -inset-x-20 -top-10 h-40 bg-brand-gradient blur-3xl opacity-30 pointer-events-none" aria-hidden />

          <div className="relative max-w-3xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-slate-200 mb-4 reveal" style={{ animationDelay: '.05s' }}>
              <span className="h-2 w-2 rounded-full bg-brand-400" />
              Live on Sui Testnet
            </div>
            <h1 className="reveal text-4xl sm:text-5xl font-semibold leading-tight text-white" style={{ animationDelay: '.1s' }}>
              Aquads — Ads on Sui, built for speed
            </h1>
            <p className="reveal text-slate-300 mt-4 text-base sm:text-lg" style={{ animationDelay: '.2s' }}>
              Monetize instantly. Bid, lock, and update creatives that go live in seconds — no intermediaries, no net‑30,
              just transparent on‑chain markets.
            </p>
            ...
            <div className="reveal mt-6 flex flex-wrap gap-3 justify-center" style={{ animationDelay: '.3s' }}>
              <Link to="/" className="btn-primary">Open Marketplace</Link>
              <Link to="/admin" className="btn-outline">For Publishers</Link>
            </div>
            <div className="reveal mt-6 grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm" style={{ animationDelay: '.35s' }}>
      ...
      <main className="mx-auto max-w-7xl px-6 py-12 space-y-16">
        <section id="features" className="grid gap-5 md:grid-cols-3">
          <FeatureCard
            ...
            body={
              <>
                Funds land in your wallet the moment bids clear. No waiting, no invoices, no opaque reporting. Spend
                less time reconciling and more time growing your audience.
              </>
            }
            delay={0.05}
          />
...
<script>window.Aquads.mount('#slot', { slotId: '0x...' })</script>`}
          ...
          <FeatureCard
            ...
            body={
              <>
                Lock a slot for a period or buy it out. Creatives update atomically and propagate instantly so your
                message is consistent everywhere, every time.
              </>
            }
            delay={0.15}
          />
        ...
        <section className="max-w-3xl mx-auto text-center">
          ...
          <p className="reveal mt-3 text-slate-300" style={{ animationDelay: '.1s' }}>
            Advertisers get a fair, open market with immediate placement and verifiable delivery. Publishers retain
            full control over inventory, reserve prices, and page targeting — with funds settling directly to their
            wallets.
          </p>
          <p className="reveal mt-3 text-slate-400" style={{ animationDelay: '.15s' }}>
            Under the hood, slots are shared objects on Sui. Events power real‑time state in our lightweight indexer,
            while the SDK fetches the latest creative for seamless rendering. It’s fast, transparent, and composable.
          </p>
        </section>

        <section id="why" className="surface p-6 text-center">
          <h2 className="text-xl font-semibold text-white reveal" style={{ animationDelay: '.05s' }}>Why Aquads</h2>
          <ul className="mt-3 grid gap-3 sm:grid-cols-2">
            {[
              ['Transparent pricing', 'On‑chain bids and events — no black boxes.'],
              ['Performance‑friendly', 'Tiny client with lazy loading and edge‑cached images.'],
              ['Publisher‑first', 'Your slots, your rules. Set reserves and page targeting.'],
              ['Dev‑ready', 'Simple HTTP APIs and a lightweight SDK.'],
            ].map(([t, d], i) => (
        ...
        <section className="text-center">
          ...
          <div className="reveal text-slate-300 mt-2" style={{ animationDelay: '.1s' }}>
            Create a slot, upload a creative, and start earning today.
          </div>
          <div className="reveal mt-6 flex justify-center gap-3" style={{ animationDelay: '.15s' }}>
            <Link to="/admin" className="btn-primary">Get Started</Link>
            <Link to="/" className="btn-outline">Browse Market</Link>
          </div>
        </section>
      </main>
    </div>
...
function FeatureCard({ icon, title, body, delay }: { icon: string; title: string; body: React.ReactNode; delay?: number }) {
  return (
