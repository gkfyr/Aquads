import { SuiClient, getFullnodeUrl, type SuiEvent } from "@mysten/sui.js/client";
...
function evTypeName(ev: SuiEvent) {
  return ev.type?.split("::").pop() || "";
}

function decodeBytesToString(v: any): string {
  try {
    if (typeof v === "string") {
      if (v.startsWith("0x")) {
...
export async function startIndexer(prisma: PrismaClient) {
  ...
  async function poll() {
    try {
      ...
      const filter = { MoveModule: { package: packageId, module: moduleName } } as any;
  ...
  async function handleEvent(e: any) {
    ...
    const slotId = (data.slot as any) ?? data.slot_id ?? "";
    ...
    if (typeName === "SlotCreated") {
      ...
      await upsertSlot({
        id: String(slotId),
        latest_meta_cid: decodeBytesToString((data as any).meta_cid),
      });
    }
  }
