...
export type SnapshotResult = {
  ...
  files: {
    ...
    screenshot: { path: string; sha256: string; size: number }
  }
  manifestPath: string
  manifestSha256: string
  blobId: string // walrus mock id
  policySig: string
...
export async function snapshotUrl(url: string, opts: { slotId?: string } = {}): Promise<SnapshotResult> {
