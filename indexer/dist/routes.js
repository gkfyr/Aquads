...
async function snapshotUrlDynamic(url, opts) {
    try {
...
function loadPagesMap() {
    try {
        if (fs.existsSync(pagesMapPath)) {
            return JSON.parse(fs.readFileSync(pagesMapPath, 'utf8'));
        }
    }
    catch { }
    return {};
}
function savePagesMap(map) {
    fs.mkdirSync(uploadsRoot, { recursive: true });
    fs.writeFileSync(pagesMapPath, JSON.stringify(map, null, 2));
}
function metaCidToPaths(metaCid) {
...
function readMetaFromCid(metaCid) {
    try {
...
function loadViewStats(slotIds) {
    const stats = {};
    if (!slotIds.length)
        return stats;
...
async function ensureSharedObject(client, id) {
    const obj = await client.getObject({ id, options: { showOwner: true } }).catch(() => null);
...
router.get("/api/slot/:id/creative/current", async (req, res) => {
    try {
        const { id } = req.params;
        const slot = await prisma.slot.findUnique({ where: { id } });
        if (!slot || !slot.latest_meta_cid)
            return res.status(404).json({ error: "No creative" });
        ...
        if (meta && typeof meta.img_cid === "string") {
            if (meta.img_cid.startsWith("mock://sha256-"))
                imgUrl = `/uploads/${meta.img_cid.replace("mock://", "")}`;
            else
                imgUrl = meta.img_cid;
        }
        res.json({
...
router.get("/api/slots", async (req, res) => {
    const { domainHash, size, website, sort } = req.query;
    const where = {};
    if (domainHash)
        where.domain_hash = String(domainHash); // legacy support
    if (size) {
    ...
    try {
        ...
        const purchasedSummaries = purchasedSlots.map((slot) => {
            ...
            for (const ev of events) {
                ...
                if (renter.toLowerCase() !== normalized)
                    continue;
...
router.post("/api/creative/default", express.json(), async (req, res) => {
    try {
        const { slotId, width, height, landingUrl, title, subtitle, bg } = req.body;
        if (!slotId)
...
router.post("/api/creative/anchor", express.json(), async (req, res) => {
    try {
        const { slotId, metaCid, checksum } = req.body;
        if (!slotId || !metaCid)
        ...
        const updateData = { latest_meta_cid: metaCid };
        ...
        res.status(500).json({ error: "anchor failed", message: e?.message || String(e) });
    }
});
...
router.post("/api/track/view", express.json(), async (req, res) => {
    try {
        const { slotId, maxPct, durationMs, ts } = req.body;
        if (!slotId)
            return res.status(400).json({ error: "slotId required" });
...
router.post("/api/snapshot", express.json(), async (req, res) => {
    try {
        const { url, slotId, anchor } = req.body;
        if (!url)
            return res.status(400).json({ error: "url required" });
        const snap = await snapshotUrlDynamic(url, { slotId });
        const result = { ok: true, snapshot: snap };
        if (anchor && slotId) {
            const client = getClient();
            const kp = keypairFromEnv(process.env.ADVERTISER_PRIVATE_KEY || "ed25519:");
            const tx = new TransactionBlock();
...
router.post("/api/tx/createSlot", express.json(), async (req, res) => {
    try {
        const { width, height, domainHash, reservePrice, pageUrl } = req.body;
        const client = getClient();
        ...
        const result = await client.signAndExecuteTransactionBlock({ signer: kp, transactionBlock: tx, options: { showEvents: true, showEffects: true, showObjectChanges: true } });
        // Try to extract created AdSlot object id from objectChanges
        const changes = result.objectChanges || [];
        const createdSlot = changes.find((c) => c.type === 'created' && typeof c.objectType === 'string' && c.objectType.endsWith('::AdSlot'));
...
router.get('/api/tx/resolveSlotId/:digest', async (req, res) => {
    try {
        ...
        const client = getClient();
        const txd = await client.getTransactionBlock({ digest, options: { showObjectChanges: true, showEffects: true, showEvents: true } });
        let slotId = null;
        const oc = txd.objectChanges || [];
        const createdSlot = oc.find((c) => c.type === 'created' && typeof c.objectType === 'string' && c.objectType.endsWith('::AdSlot'));
        slotId = createdSlot?.objectId || null;
        if (!slotId) {
            const evs = txd.events || [];
            const sc = evs.find((e) => typeof e.type === 'string' && e.type.endsWith('::SlotCreated') && e.parsedJson?.slot);
            slotId = sc?.parsedJson?.slot || null;
        }
        if (!slotId) {
            const effCreated = txd.effects?.created || [];
            slotId = effCreated[0]?.reference?.objectId || null;
        }
        res.json({ slotId, tx: { digest, hasObjectChanges: Boolean(txd.objectChanges), hasEvents: Boolean(txd.events) } });
    }
    catch (e) {
...
router.post("/api/tx/lock", express.json(), async (req, res) => {
    try {
        const { slotId, amount, lockSecs } = req.body;
        const client = getClient();
        await ensureSharedObject(client, slotId);
        const kp = keypairFromEnv(process.env.ADVERTISER_PRIVATE_KEY || "ed25519:");
        const tx = new TransactionBlock();
